...
- **Mini-Player Synced Lyrics Crossfade:** Added an optional synced-lyrics mode for the mini-player with persistent user preference (`miniplayer_lyrics_enabled`), one-time-per-track lyric parsing, and smooth `AnimatedContent` crossfades between title/artist metadata and the active lyric line.
- **Mini-Player Lyrics Sync/Source Parity Fixes:** Mini-player lyrics now follow main-player behavior by honoring Romanized Lyrics mode, applying `lyricsOffsetMs` with the same parser path as `LyricsOverlay`, increasing progress refresh cadence for near-realtime line updates, showing compact song metadata alongside active lyric lines, and falling back to title/artist while paused.
